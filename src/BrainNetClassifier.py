...
class BrainNet:
	...
	def get_model(self, X, reuse=False):
		with slim.arg_scope([slim.layers.conv2d, slim.layers.fully_connected],
			...
			net = slim.layers.fully_connected(net, self.num_output, scope='fc4')
			net = slim.fully_connected(net, self.num_classes, scope='fc5', activation_fn=tf.nn.softmax, trainable=False)
			return net
